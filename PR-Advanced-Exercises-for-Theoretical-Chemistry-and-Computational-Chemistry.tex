...
\hypersetup{
  pdftitle={PR Advanced Exercises for Theoretical Chemistry and Computational Chemistry},
  colorlinks=true,
  linkcolor={highlight},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{PR Advanced Exercises for Theoretical Chemistry and Computational
Chemistry}
\author{}
\date{}
...
Welcome to the instruction page of the 724651 PR Advanced Exercises for
...
checklist which results should be included on the poster.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-pdf/cell-2-output-1.pdf}}

}

\caption{Example code with corresponding plot. Any similarity to the
greek letter Ψ is pure coincidence ;)}

\end{figure}%

\part{Exercise: Vibrational Spectroscopy}

\section{Background Info}\label{background-info}

\section*{Exercise -- Vibrational Spectroscopy using Nuclear Wave
Functions}\label{exercise-vibrational-spectroscopy-using-nuclear-wave-functions}
\addcontentsline{toc}{section}{Exercise -- Vibrational Spectroscopy
using Nuclear Wave Functions}

\markboth{Exercise -- Vibrational Spectroscopy using Nuclear Wave
Functions}{Exercise -- Vibrational Spectroscopy using Nuclear Wave
Functions}

The simplest way to generate vibrational spectra is the \textbf{harmonic
approximation}: all second derivatives of the energy with respect to the
nuclear degrees of freedom are computed at a pre-optimized minimum
structure, yielding the Hessian: \[
H_{ij} = \frac{\partial^2 \left < E \right >  }{\partial q_i \partial q_j}
\] After extraction of the force constants \(k\) of each vibrational
mode from the Hess-matrix (via diagonalisation), the harmonic
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{exercise_a/../img/vibrational.png}

with \(\mu\) being the associated reduced mass. However, a short-coming
of this approach is the \textbf{neglect of anharmonicity}, \textbf{mode
coupling} and \textbf{nuclear quantum effects}. In some cases these
contributions cancel, while for other systems the error in the harmonic
frequencies can be up to \(300\,\mathrm{cm^{-1}}\).

An elegant alternative is the calculation of vibrational wave functions
by numerically solving the nuclear Schrödinger equation. One method to
achieve this is a method according to \textbf{Boris V. Numerov}
requiring the potential curve as input in form of a discretised grid.
The latter can be obtained from a potential energy scan executed for
instance using the QM program \texttt{gaussian}. The difference between
the eigenvalue of the ground state (\(n=0\)) and the first (\(n=1\)) and
second (\(n=2\)) excited states correspond to the expected vibrational
frequency of the fundamental and first overtone, when divided by
Planck's constant:
...
From the frequencies \(\nu_{n0}\) (in \(\mathrm{s}^{-1}\)) the
respective wave numbers \(\bar{\nu}_{n0}\) (in \(\mathrm{cm}^{-1}\))
have to be calculated and reported. Knowledge of the wave function also
enables the calculation of the oscillator strengths \(f_{n0}\),
providing an accurate estimate of the expected IR absorption
intensities.
...
To calculate the oscillator strength, the wave numbers
\(\bar{\nu}_{n0}\) and the respective transition dipole moment
\(\mu_{n0}\) is required. The latter is given as:
...
While this integral appears intimidating, it can be easily evaluated in
an approximate way using the trapezoidal rule:
...
The calculation of the trapezoidal integration is actually very simple
and can be easily implemented in a spreadsheet editor such as
\texttt{libreoffice} or \texttt{excel} (or computed using data science
software such as \texttt{matlab}, \texttt{octave},\texttt{python},
\texttt{R}, etc.)

\subsection*{Diatomic molecule}\label{diatomic-molecule}
\addcontentsline{toc}{subsection}{Diatomic molecule}

\markright{Diatomic molecule}

In this part the vibration of a diatomic molecule is investigated. After
drawing the molecule using \texttt{gaussview}, an energy minimisation (=
geometry optimisation) and a harmonic frequency calculation has to be
carried out. In the next step a potential energy scan has to be
performed in the range of approx. -0.5\(\,\)Å to +1.5\(\,\)Å using a
step length \(\Delta r = 0.01\,\)Å. This yields about \(150\) individual
potential points.

The energy and dipole moment information can be extracted in the
terminal using the provided tool:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{./extract\_data.sh}\NormalTok{  scan{-}logfile   }\OperatorTok{\textgreater{}}\NormalTok{  data{-}file}
\end{Highlighting}
\end{Shaded}

The data file contains the distance information in Å, the QM energy in
Hartree, and the components of the dipole moment in \(x\), \(y\) and
\(z\) direction in Debye. Next, the force constant \(k\) according to
the harmonic approximation has to be determined via finite differences
from the energy column of the data-file (second column).

\[
k \approx \frac{E_1 -2 E_{\mathrm{min}} + E_{+1}}{\Delta r^2}
\]

with \(E_{\mathrm{min}}\) being the minum energy and \(E_{\pm 1}\)
corresponding to the energy values of the two neighbouring points.
Please report the force constant in
\(\mathrm{kcal\cdot mol^{-1}Å^{-2}}\):

Because the program gaussian provides a wrong reduced mass \(\mu\) in
case of diatomic systems, this property has to be calculated manually in
...
Using the reduced mass \(μ\) and the force constant \(k\) the frequency
(in \(\mathrm{s^{-1}}\)) and corresponding wave number (in
\(\mathrm{cm^{-1}}\)) should be calculated and compared to the result
obtained from gaussian.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, colback=white]

Extra care is required to apply all factors to obtain the correct unit
of the frequency and wavenumber (i.e.~just dividing the force constant
\(k\) in \(\mathrm{kcal\cdot mol^{-1}Å^{-2}}\) by the reduced mass
\(\mu\) given in \(\mathrm{g \cdot mol^{-1}}\) will obviously yield a
wrong result \ldots)

\end{tcolorbox}

In the final step the Numerov calculation is carried out, requiring the
data file and the reduced mass as input:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{./numerov}\NormalTok{    data{-}file   mass   }\OperatorTok{\textgreater{}}\NormalTok{  numerov{-}outfile}
\end{Highlighting}
\end{Shaded}

The output of the Numerov procedure contains the coordinate and
potential information (in units Å and \(\mathrm{kcal\cdot mol^{-1}}\))
as well as the wave-functions for the five lowest states. The energy
eigenvalues are given as comment line (also in
\(\mathrm{kcal\cdot mol^{-1}}\)) on top of the Nuermov output file. The
respective energy differences have to be transformed into
\(\mathrm{cm^{-1}}\).

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={The report for this part should include:}, opacityback=0, colback=white]
...
\item
  The calculation of the force constant \(k\) and the reduced mass
  \(\mu\).
\item
  Calculation of the frequency and wave number showing ALL(!) required
  conversion factors.
\item
  A figure showing the QM energy as a function of distance and the
  Numerov wave functions plus the energy graph showing the harmonic
  approximation (all in the same figure).
\item
  A table comparing the wavenumbers obtained via the Numerov procedure
  with the results obtained from the harmonic approximation for \(n\) =
  1 - 4.
\item
  For all points a short description/interpretation should be included
  as text.
\end{itemize}
...
In this part, the transition dipole moments and oscillator strengths are
\textbf{NOT(!)} required.

\end{tcolorbox}

\subsection*{Stretch Vibration in a Polyatomic
Molecule}\label{stretch-vibration-in-a-polyatomic-molecule}
\addcontentsline{toc}{subsection}{Stretch Vibration in a Polyatomic
Molecule}

\markright{Stretch Vibration in a Polyatomic Molecule}

In this example, the X-H stretch motion of a polyatomic molecule (X=O,N)
provides a suitable approximation to the actual normal mode. In order to
assess the influence of quantum effects, both hydrated and deuterated
species (i.e.~X-H and X-D) have to be investigated. After the geometry
optimisation and frequency computation, the potential energy scan of the
X-H distance in the range of approx. \(0.5\,\mathrm{Å}\) to
\(2.0\,\mathrm{Å}\) has to be carried out, using again a step length of
\(0.01\,\mathrm{Å}\).

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-warning-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Careful:}, opacityback=0, colback=white]

Be sure to select the correct bond for the potential energy scan! (This
is a \textbf{very} frequent error in this excercise.)

\end{tcolorbox}

As before the energy and dipole moment information can be extracted in
the terminal using the provided tool:
...
The resulting data file contains the coordinate, energy and dipole
moment information (in units of Å, Hartree and Debye) and can be
directly used as input for the Numerov programm:
...
In addition to the data file, the reduced mass of the vibrational mode
has to be provided as a second input. For polyatomic molecules the
reduced mass provided in the gaussian output file is correct! To
identify the correct mode, a visual inspection in \texttt{gaussview} is
required.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-warning-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Careful:}, opacityback=0, colback=white]

Be sure to make a separate check in the deuterated case -- the order of
vibrations is sometimes different (again a \textbf{very} common error in
this excercise).

\end{tcolorbox}

As before the output of the Numerov procedure contains the coordinate
and potential information (in units Å and
\(\mathrm{kcal\cdot mol^{-1}}\)) as well as the wave-functions for the
five lowest states. The energy eigenvalues are given as comment line
(also in \(\mathrm{kcal\cdot mol^{-1}}\)) on top of the file. The
respective energy differences have to be transformed into
\(\mathrm{cm^{-1}}\). Prior to computing the transition dipole moments
it should be verified that the wave functions are orthonormal for the
states \(i, j\) = 0 -- 2.
...
Next, the oscillator strength \(f_{n0}\) (for \(n=1,2\)) should be
computed from the transition dipole moments and reported relative to
\(f_{10}\). (\emph{i.e.} the oscillator strengths are divided by
\(f_{10}\), thus always giving a number of \(1.0\) for the ground state
transition).
...
\item
  Two figures comparing the potential energy and the first five wave
  functions including also the potential energy of the harmonic
  approximation for both the hydrated and deuterated species.
\item
  A graph showing the components of the dipole moment as a function of
  the distance.
\item
  Results of the orthonormality check between the states \(i\),
  \(j = {0,1,2}\) in form of a table.
\item
  A table comparing the frequencies and oscillator strengths of the
  fundamental and first overtone modes obtained via the Numerov
  procedure and the harmonic approximation.
\item
  For all points a short description/interpretation should be included
  as text.
\end{itemize}
...
\addcontentsline{toc}{subsection}{Protocol:}
...
Atrium of the CCB building), see \href{../templates/poster.qmd}{poster
templeates}. All data generated along with a discussion of results fit
easily on a single page. Please include your name and matriculation
number, and provide the protocol as pdf-file including your surname in
the name of the file.

\section{HowTo}\label{howto}

The required scripts and programs for this exercise are provided in the
folder:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{cd}\NormalTok{ /media/TC\_Lehre/03\_PR\_Fortgeschrittene\_Uebungen\_zu\_TC\_und\_Comp\_Chemie/vibration/}
\end{Highlighting}
\end{Shaded}
...
If you are unsure please look at this
\href{../guides/linux_quickstart_guide.qmd}{Linux quickstart guide}.

\subsubsection*{\texorpdfstring{Vibrational spectroscopy of a diatomic
molecule at the example of
\(\mathrm{H_2}\)}{Vibrational spectroscopy of a diatomic molecule at the example of \textbackslash mathrm\{H\_2\}}}\label{vibrational-spectroscopy-of-a-diatomic-molecule-at-the-example-of-mathrmh_2}
\addcontentsline{toc}{subsubsection}{Vibrational spectroscopy of a
diatomic molecule at the example of \(\mathrm{H_2}\)}
...
\tightlist
\item
  Execute an energy minimisation, a frequency calculation and a
  potential energy scan using \texttt{gaussian}
\item
...
\item
  Calculation of the harmonic vibrational wave numbers from the force
  constant (compare this value to the gaussian result for verification
  -- deviation typically about 1-2 \(\mathrm{cm^{-1}}\))
\item
  Calculation of the anharmonic vibrational wave numbers from the
  Numerov eigenvalues
\item
  1 plot \(V_{\mathrm{QM}}\) vs bond length including the vibrational
  wave function from the Numerov output and the harmonic approximation
  to the potential energy
\item
  Table comparing the harmonic and anharmonic wave numbers for the
  states 01 to 04
\item
  A screenshot of the equilibrium geometry (use a white background
  color!)
\end{itemize}

\subsubsection*{HowTo}\label{howto-1}
\addcontentsline{toc}{subsubsection}{HowTo}

\textbf{1)} Draw the structure using \texttt{gaussview} (command
\texttt{gv}) and save the file, \emph{e.g.} as \texttt{h2\_opt.com}.
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={GaussView}, opacityback=0, colback=white]

draw stuff\\
File \textgreater{} Save\\
Be sure to \textbf{NOT} save cartesian coordinates, so ``Write
Cartesian'' must be unticked!

\end{tcolorbox}

\textbf{2)} Open the File in your favourite editor (\texttt{code},
\texttt{nedit}, \texttt{gedit}, \texttt{vi}, \ldots) and change the
command line (\texttt{\#}) to

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={h2\_opt.com}, opacityback=0, colback=white]

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# B3LYP/6{-}311++G(3df,3pd) OPT=TIGHT SCF=VERYTIGHT INT=ULTRAFINE}
\end{Highlighting}
\end{Shaded}

\end{tcolorbox}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, opacityback=0, colback=white]

It is important to include \textbf{one (and only one) blank line} after
the command line (\texttt{\#}-line), the title-line as well as the atom
list and at least one blank-line after the last input for all
\texttt{gaussian} input file!

\end{tcolorbox}

\textbf{3)} Execute the geometry optimisation:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{g16}\NormalTok{ h2\_opt.com}
\end{Highlighting}
\end{Shaded}

\textbf{4)} Open the log-file (\texttt{code},\texttt{nedit},
\texttt{gedit}, \texttt{vi}, \ldots) and get the equilibrium bond
distance,\emph{e.g.}:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nedit}\NormalTok{ h2\_opt.log}
\end{Highlighting}
\end{Shaded}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={h2\_opt.log}, opacityback=0, colback=white]
...
\textbf{5)} Copy the in-file and change the command line to that of a
frequency calculation, \emph{e.g.}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cp}\NormalTok{ h2\_opt.com h2\_freq.com}
\end{Highlighting}
\end{Shaded}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={h2\_freq.com}, opacityback=0, colback=white]

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# B3LYP/6{-}311++G(3df,3pd) FREQ SCF=VERYTIGHT INT=ULTRAFINE}
\end{Highlighting}
\end{Shaded}

\end{tcolorbox}

Also set the bond length to the minimum distance, \emph{e.g.}
...
\textbf{6)} Execute the frequency calculation and get the harmonic
frequency from the log-file

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{g16}\NormalTok{ h2\_freq.com}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{nedit}\NormalTok{ h2\_freq.log}
\end{Highlighting}
\end{Shaded}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={h2\_freq.log}, opacityback=0, colback=white]
...
\strut ~~~~~~~~ 1\\
\strut ~~~~~~~~ SGG\\
Frequencies -\/- ~~~~~~~ { \textbf{4410.4726} }\\
Red. masses -\/- ~~~~~~~~~~~~ { \st{1.0078} }\\
Frc consts -\/- ~~~~~~~~~~ ~ ~~ ~~ { \st{11.550} }\\
IR Inten -\/- ~~~~~~~~~~~~~~~~ ~~~~~ { 0.0000 }\\
{[}\ldots{]}\\
{[}\ldots{]}\\
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Be careful}, opacityback=0, colback=white]

For diatomics the force constant and the reduced mass are not given
correctly! (according to the developers that's a feature, not an error
\ldots) This is the reason why in this exercise, the force constant has
to be determined from the potential energy scan! Do \textbf{NOT} use the
force constant from the output file.

\end{tcolorbox}

\textbf{7)} Copy the in-file and change the command line for the
potential energy scan, \emph{e.g.}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cp}\NormalTok{ h2\_freq.com h2\_scan.com}
\end{Highlighting}
\end{Shaded}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={h2\_scan.com}, opacityback=0, colback=white]

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# B3LYP/6{-}311++G(3df,3pd) SCAN SCF=VERYTIGHT INT=ULTRAFINE }
\CommentTok{\# GEOM=NOCROWD NOSYMM POP=ALWAYS}
\end{Highlighting}
\end{Shaded}

\end{tcolorbox}

\textbf{8)} Change the starting value of the bond length by subtracting
\(\mathrm{0.5\,Å}\), and set up the scan points \emph{e.g.}:

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={Note}, opacityback=0, colback=white]
...
This will perform a potential energy scan from
\(\mathrm{0.2427352\,Å}\), increasing the H-H distance \(150\) times by
\(\mathrm{0.01\,Å}\), thus the scan will end at a H-H distance of
\(\mathrm{1.7427352\,Å}\). Don't forget to include a final blank line in
the gaussian input.

\textbf{9)} Execute the potential energy scan and grab a coffee:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{g16}\NormalTok{ h2\_scan.com}
\end{Highlighting}
\end{Shaded}

\textbf{10)} Copy the extraction-script and the Numerov analysis code to
your working directory:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cp}\NormalTok{ /media/TC\_Lehre/03\_PR\_Fortgeschrittene\_Uebungen\_zu\_TC\_und\_Comp\_Chemie/vibration/extract\_data.sh .}
\FunctionTok{cp}\NormalTok{ /media/TC\_Lehre/03\_PR\_Fortgeschrittene\_Uebungen\_zu\_TC\_und\_Comp\_Chemie/vibration/numerov .}
\end{Highlighting}
\end{Shaded}

\textbf{11)} Extract the potential energy and dipole moment information
from the log-file and use \texttt{\textgreater{}} to redirect the
output:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{./extract\_data.sh}\NormalTok{ h2\_scan.log }\OperatorTok{\textgreater{}}\NormalTok{ scan\_data.dat}
\end{Highlighting}
\end{Shaded}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-warning-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, opacityback=0, colback=white]

Careful -- if told, the redirect \texttt{\textgreater{}} will overwrite
existing files and there is no undo if this happens!

\end{tcolorbox}

The file \texttt{scan\_data.dat} contains five data columns:

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={scan\_data.dat}, opacityback=0, colback=white]
...
\item
  columns \(3\) to \(5\) : \(x\), \(y\) and \(z\)-component of the
  dipole moment
\end{itemize}

\end{tcolorbox}

This file can be directly imported into graphic-programs, \emph{e.g.}
\texttt{office},\texttt{xmgrace}, \texttt{gnuplot}, \ldots{}

The dipole moment data is only required in part
\href{exercise_a.qmd}{A.2} of this practical course.

\textbf{12)} Calculate the reduced mass of the vibration and execute the
Numerov analysis:

\begin{itemize}
\tightlist
\item
  In this course always the reduced mass formula for two particles
  should be used.
\end{itemize}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-warning-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, opacityback=0, colback=white]

Be sure to use the mass of the correct isotopes, \emph{e.g.}
\(\mathrm{^1H_2}\), \(\mathrm{^2D_2}\), \(\mathrm{^{12}C}\),
\(\mathrm{^{18}O}\) and do \textbf{NOT} use the averaged mass given in
the periodic table.
...
\tightlist
\item
  The Numerov-program requires two inputs, being the data-file from step
  11 above and the reduced mass, e.g.~in case of \(\mathrm{^1H_2}\):
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{./numerov}\NormalTok{ scan\_data.dat 0.50391251605 }\OperatorTok{\textgreater{}}\NormalTok{ numerov.dat}
\end{Highlighting}
\end{Shaded}

Again, be careful while redirecting \texttt{\textgreater{}} to not
overwrite any files.

The file \texttt{numerov.dat} contains a headline (\texttt{\#}) and
twelve data columns. Columns \(1\) to \(7\) are to generate the required
figure, columns \(8\) to \(12\) are for data analysis:

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={numerov.dat}, opacityback=0, colback=white]

\begin{itemize}
\tightlist
\item
  \# headline -- energy eigenvalues E of the five lowest wave functions
  \(\Psi_0\) to \(\Psi_4\) in \(\mathrm{kcal/mol}\)
\item
  column \(1\) : Distance in Å\\
\item
  column \(2\) : Energy in \(\mathrm{kcal/mol}\) and shifted to zero at
  the minimum
\item
  columns \(3\) to \(7\) : wave functions \(\Psi_0\) to \(\Psi_4\)
  shifted by the respective eigenvalue
\item
  columns \(8\) to \(12\): wave functions \(\Psi_0\) to \(\Psi_4\)
  without shift
\end{itemize}

\end{tcolorbox}

This file can be directly imported into graphic programs (e.g.~office,
xmgrace, gnuplot, \ldots) as well as data software (\texttt{office},
\texttt{matlab}, \texttt{octave}, \texttt{R}, \texttt{bash},
\texttt{origin}, \ldots) to perform the analyses required in
\href{exercise_a.qmd}{A.2.2}.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, colback=white]

Which software you choose for the analysis part (or if you want to write
your own code) is entirely up to you. There are different ways to obtain
the requested data, \emph{e.g.} the numerical integration can be carried
out i) in an \texttt{excel}-sheet, ii) using \texttt{Python}/\texttt{R}
datascience routines, iii) on the command line using a (very simple)
\texttt{awk}-script or iv) even graphically in the program
\texttt{xmgrace}. You can choose whichever method works best for you.

\end{tcolorbox}

\subsubsection*{Stretch Vibration in a Polyatomic
Molecule}\label{stretch-vibration-in-a-polyatomic-molecule-1}
\addcontentsline{toc}{subsubsection}{Stretch Vibration in a Polyatomic
Molecule}
...
\item
  Execute an energy minimisation, a frequency calculation and a
  potential energy scan using gaussian
\item
  Extract the potential energy data from the output file
\item
  Execute a Numerov calculation
\item
  Analyse both a hydrated (\(\mathrm{^1H}\)) and deuterated system
  (\(\mathrm{^2D}\))
\end{enumerate}
...
\item
  Calculation of the harmonic vibrational wave numbers from the force
  constant for \(\mathrm{^1H}\) and \(\mathrm{^2D}\) (compare these
  values to the gaussian results for verification -- deviation typically
  about \(1-2\) \(\mathrm{cm^{-1}}\))
\item
  Calculation of the anharmonic vibrational wave numbers from the
  Numerov eigenvalues
\item
  2 plots \(V_{\mathrm{QM}}\) vs bond length including the vibrational
  wave function from the Numerov output and the harmonic approximation
  to the potential energy
\item
  1 plot showing the components of the dipole moment and the total
  dipole moment as a function of the distance.
\item
  Results of the orthonormality check between the states \(i,j = 0,1,2\)
  should be provided in a table.
\item
  A table comparing the frequencies and oscillator strengths of the
  fundamental and \(1^{\mathrm{st}}\) overtone modes obtained via the
  Numerov procedure and the harmonic approximation should be included.
\item
  A screenshot of the equilibrium geometry (use a white background
  color!)
\end{itemize}

Follow steps 1) to 12) of part 2.1 for the polyatomic system. Redo the
calculations for the deuterated system, but be smart - not all
calculations of the steps i) to iii) above have to be repeated for the
deuterated system. Just consider which calculation steps are
mass-independent and which one is not.

In order to change the isotope from \(\mathrm{^1H}\) and
\(\mathrm{^2D}\) in the gaussian calculation, the iso-keyword should be
applied, \emph{e.g.} in case of a frequency calculation for
\(\mathrm{D_2}\):

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title={d2\_scan.dat}, opacityback=0, colback=white]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Common errors/pitfalls in this exercise}, opacityback=0, colback=white]

\begin{itemize}
\tightlist
\item
  Be \textbf{VERY} careful which bond to scan -- in many cases the
  correct bond is NOT \texttt{B1} (a very common error!). Which bond to
  scan depends on the way the molecule is drawn and the resulting order
  of atoms.
\item
  If you start to draw the molecule starting with the functional group
  of interest (\emph{e.g.} start with the OH- group in case of
  methanol), the y-component of the dipole moment will be zero. This
  means less work in the analysis of the transition dipole moment and
  the oscillator strengths.
\item
  Be aware of the units of the different properties and take your time
  to perform a dimensional analysis. For instance, typical units of the
  force constants k and the reduced mass \(\mu\) are
  \(\mathrm{kcal \cdot mol^{-1}\cdot Å^{-2}}\) and \(\mathrm{g/mol}\),
  respectively. It is not possible to simple use the associated values
  to calculate the frequency without first making the units compatible:
  \[
  \nu = \frac{1}{2\pi}\sqrt{\frac{k}{\mu}}
  \]
\end{itemize}

Here, the units \(\mathrm{kcal}\), \(\mathrm{Å^{-2}}\) and
\(\mathrm{g}\) need to be converted to obtain the unit of
\(\mathrm{s^{-1}}\) for the frequency.
...
your surname in the name of the file.
...
\item[$\square$]
  ...
  \(\mu\).
\item[$\square$]
  Calculation of the frequency and wave number showing \textbf{ALL(!)}
...
\item[$\square$]
  ...
  the distance.
\item[$\square$]
  Results of the orthonormality check between the states
  \(i, j = {0,1,2}\) in form of a table.
\item[$\square$]
  A table comparing the frequencies and oscillator strengths of the
  fundamental and first overtone modes obtained via the Numerov
  procedure and the harmonic approximation.
...
In this part, the transition dipole moments and oscillator strengths
\textbf{ARE(!)} required for the analysis.

\end{tcolorbox}

\part{Exercise: NNP-MD}

\section{Background Info}\label{background-info-1}

\section{\texorpdfstring{Exercise - NNP MD Simulations of
CO\textsubscript{2}@ZIF-8}{Exercise - NNP MD Simulations of CO2@ZIF-8}}\label{exercise---nnp-md-simulations-of-co2zif-8}
...
compounds such as zeolites (\emph{i.e.} aluminosilicates) or polymers
lies in their crystalline nature, which allows for systematic
modification through crystal engineering principles by altering their
organic linkers or metal nodes. This capability leads to an almost
limitless number of possible framework topologies and properties, making
them particularly appealing for technological applications. In 2025, the
\href{https://www.nobelprize.org/prizes/chemistry/2025/summary/}{Nobel
Prize in Chemistry} was awarded to Profs. Susumu Kitagawa, Richard
Robson and Omar M. Yaghi ``for the developement of metal-organic
frameworks''.
...
of critical green house gases such as CO\textsubscript{2} and
CH\textsubscript{4} are widely discussed {[}2,3{]}. In addition, due to
their ultrahigh internal surface area, tunable pore dimensions, and
rapid kinetics for gas adsorption and desorption, MOFs have emerged as
highly promising candidates for capturing and storing carbon dioxide,
called direct air capture (DAC) {[}4{]}.

An increasing number of MOF compounds are discussed as potential host
matrices for green house gases {[}4{]}. In this exercise, carbon dioxide
storage in the comparably simple ZIF-8 (zeolitic imidazolate framework)
is investigated via molecular dynamics simulations (MD), to keep the
computational effort and memory demand manageable.

The ZIF-8 system Zn(2-methylimidazolate)\textsubscript{2} crystallizes
in the non-centrosymmetric cubic space group I43m (space group no. 217)
with a lattice parameter of approx 1.7~nm. The cubic unit cell contains
a total of 12 Zn\textsuperscript{2+} cations that are each tetrahedrally
coordinated by four 2-methylimidazolate linkers. The pore structure of
ZIF-8 is similar to the topology of a prototypic sodalite zeolite (hence
the name zeolitic imidazolate framework). Several studies have
investigated ZIF systems with respect to their CO\textsubscript{2}
storage capacity {[}5-8{]}.

In order to achieve fast and accurate MD simulations, the MACE-MP neural
network potential (NNP) {[}9-11{]} is applied, in particular the
MACE-DAC-1 model {[}12{]}, which is trained not only on solid-state
systems but also on interactions with gases for direct air capture (DAC)
applications. If trained properly, NNPs provide an efficient and
versatile description of chemical systems, combining the accuracy of
high level quantum chemical methods with the speed of classical force
field approaches.

In this exercise the properties of the pristine host material (lattice
parameter and thermal expansion coefficient) and the interaction between
the inserted CO\textsubscript{2} molecules and the ZIF-8 host (diffusion
coefficient and activation energy of diffusion) will be studied.
...
frameworks (MOFs) for CO\textsubscript{2} capture at different
pressures'' J. Environ. Chem. Eng. 2022, 10, 108930/1 -- 108930/35 DOI:
10.1016/j.jece.2022.108930

{[}5{]} Abraha, Yuel W.; Tsai, C.-W.; Niemantsverdriet, J. W. H.;
Langner E. H. G. ''Optimized CO\textsubscript{2} Capture of the Zeolitic
Imidazolate Framework ZIF-8 Modified by Solvent-Assisted Ligand
Exchange'' ACS Omega 2021, 6, 21850 -- 21860 DOI:
10.1021/acsomega.1c01130

{[}6{]} Jiang, S.; Liu, J.; Guan J.; Du, X.; Chen, S.; Song, Y.; Huan,
Y. ''Enhancing CO\textsubscript{2} adsorption capacity of ZIF‑8 by
synergetic effect of high pressure and temperature'' Sci. Rep.~2023,
17584/1 -- 17584/8 DOI:~10.1038/s41598-023-44960-4

{[}7{]} Kalauni, K.; Vedrtnam, A.; Wdowin, M.; Chaturvedi, S. ``ZIF for
CO\textsubscript{2} Capture: Structure, Mechanism, Optimization, and
Modeling'' Processes 2022, 10, 2689/1 -- 2689/32 DOI: 10.3390/pr10122689
...
{[}12{]} Yunsung L., Hyunsoo P., Aron W., Jihan K. ``Accelerating CO2
direct air capture screening for metal-organic frameworks with a
transferable machine learning force field'' Matter 2025, 8, 102203 DOI:
10.1016/j.matt.2025.102203

\section{HowTo}\label{howto-2}
...
Each student will be assigned two specific temperature values and a
specific carbon dioxide loading for the simulations. One temperature
...
All tools and programs are provided by the HoferLab™. The simulation
engine that will be used is \href{https://github.com/MolarVerse/PQ}{PQ}.
The analysis tools are written in Python and C, and are partially based
on the \href{https://github.com/MolarVerse/PQAnalysis}{PQAnalysis}
library.
...
The directories encoded in the \texttt{ZIF\_DIR} variable contain all
necessary files for the simulations. The files are organized in the
following way:

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{File Structure}, opacityback=0, colback=white]
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.1594}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.8406}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Folder/File
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\texttt{preeq} & Contains the pre-equilibrated restart-files
(\texttt{zif8\_preeq\_*.rst}) and associated topology files
...
dioxide molecules in ZIF-8. There are a total of five different
temperatures (298.15 K, 323.15 K, 348.15 K, 373.15 K, 398.15 K) and five
different CO\textsubscript{2} loadings (6, 8, 10, 12, 14). Each student
is carrying out one CO\textsubscript{2}@ZIF-8 simulation at the assigned
temperature with the assigned loading.
...
\textbf{2.} Add the CO\textsubscript{2} loading to restart files
(e.g.~32 carbon dioxide molecules and 298 K)

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{add\_molecules}\NormalTok{ zif8\_preeq\_298.rst co2.rst }\AttributeTok{{-}{-}rst{-}mol{-}desc{-}file}\NormalTok{ moldescriptor.dat }\AttributeTok{{-}n}\NormalTok{ 32 }\OperatorTok{\textgreater{}}\NormalTok{ 32xco2{-}zif8{-}00.rst}
\end{Highlighting}
\end{Shaded}
...
\textbf{1.} Edit the input files \texttt{run-01.in} to
\texttt{run-04.in} to include your assigned temperature, generated
restart file, and topology file.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, opacityback=0, colback=white]

Please make sure and double check that all input files \texttt{.in} are
edited accordingly and no typos are present! Otherwise, PQ won't be able
to read in the input files and the simulation will stop prematurely!
...
start file and \texttt{-01} as the file prefix in \texttt{run-01.in} and
increment the numbers by one for each following input file. This will
help to keep track of the different simulation steps.

\end{tcolorbox}

\textbf{2.} Run NVT equilibration
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{PQ}\NormalTok{ run{-}01.in }\KeywordTok{\&\&} \ExtensionTok{PQ}\NormalTok{ run{-}02.in }\KeywordTok{\&\&} \ExtensionTok{PQ}\NormalTok{ run{-}03.in }\KeywordTok{\&\&} \ExtensionTok{PQ}\NormalTok{ run{-}04.in}
\end{Highlighting}
\end{Shaded}
...
Molecules}\label{extract-carbon-dioxide-molecules}
\addcontentsline{toc}{subsection}{Extract Carbon Dioxide Molecules}

\markright{Extract Carbon Dioxide Molecules}

Extract the carbon dioxide molecules from the trajectory and velocity
...
For the assigned carbon dioxide loading, please insert the calculated
self-diffusion coefficient value into the \textbf{Google Sheet}.
...
For the assigned carbon dioxide loading, insert the calculated
self-diffusion coefficient value into the \textbf{Google Sheet}.
...
\item
  Render the image:
  \texttt{File\ \textgreater{}\ Render\ \textgreater{}\ Tachyon}
\end{itemize}
...
the pristine ZIF-8 framework at the assigned temperature value.
...
\textbf{1.} Edit the input files \texttt{run-01.in} to
\texttt{run-03.in} to include your assigned temperature, generated
restart file, and topology file. \texttt{run-04.in} is not needed for
this part of the exercise.
...
10 ps NVT equilibration (\texttt{run-01.in}). The second stage is a 10
ps NPT equilibration (\texttt{run-02.in}). After the equilibration, the
production run is performed for 500 ps (\texttt{run-03.in}). Only the
trajectory from the last run \texttt{run-03.in} is used for the thermal
...
For the assigned temperature, insert the calculated average lattice
parameter and average volume into the \textbf{Google Sheet}.

\end{tcolorbox}

\textbf{3.} Calculate the thermal expansion coefficient using the values
from your group and the following formula:
\hyperref[linear--volumetric-thermal-expansion-coefficient]{thermal
expansion}.

\textbf{4.} Compare the calculated thermal expansion coefficient of
ZIF-8 with literature values.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, colback=white]

Include the source of the reference on the poster and which method they
used to obtain it. \href{https://scholar.google.com/}{Google Scholar} is
your friend for this research ;)
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-important-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{An equilibrium state:}, opacityback=0, colback=white]

can be defined as a state in which macroscopic properties \(A\)
(\emph{e.g} temperature \(T\), pressure \(P\), energies \(E\), volume
...
The running average \(\left < A \right >_{\tau}\) at time \(\tau\) is
calculated by averaging the previous \(\omega\) values, sampled at
intervals of \(g\):

\[
\left < A \right >_{\tau} = \frac{1}{\omega}\sum_{i=0}^{\omega - 1} a(\tau - i \cdot g)
\] \emph{e.g.} \(\tau=10\), \(\omega=3\), \(g=2\) \[
\left < A \right >_{10} = \frac{1}{3} (a(t_{10})+a(t_{8})+a(t_{6}))
\]

It smooths the signal by reducing short-term fluctuations while
retaining the overall trend of the data.

\subsection{Cumulative Average}\label{cumulative-average}

At each step, the cumulative average takes the previous average
\(\left < A \right >_{n-1}\) and adjusts it to account for the new data
point \(x_n\): \[
\left < A \right >_n = \frac{(n-1)\left < A \right >_{n-1} + x_n}{n}
\]

\subsection{Linear Regression Fit}\label{linear-regression-fit}

A linear regression fit can help to estimate the trend of your
simulation.

For further detail: \href{../guides/statistics.qmd}{statistics}

\subsection{Plot of temperature over simulation
time}\label{plot-of-temperature-over-simulation-time}

The temperature during the simulation can be found in the associated
energy file \texttt{*.en}, which can be readily used for plotting. The
\texttt{*.info} file contains information about the individual columns
in the \texttt{*.en} file. An example code to plot the temperature vs
simulation time is shown below.
...
\pandocbounded{\includegraphics[keepaspectratio]{exercise_b/analysis_b_files/figure-pdf/fig-temp-output-2.pdf}}
...
extent to which a solid substance undergoes a change in size in response
to variations in temperature.

Therefore the system is computed in the \(NPT\) ensemble at different
temperatures \(T_i\) at \(1\,\mathrm{bar}\).

In our case we consider a temperature range from
\(248\,\text{to}\,348\,\mathrm{K}\) in \(25\,\mathrm{K}\) steps.
...
The linear thermal expansion coefficient at \(\mathrm{298}\,\mathrm{K}\)
is calculated from the temperature gradient of the respective average
lattice parameter \(\left < a \right >\), \(\left < b \right >\) or
...
fundamental types of finite differentiation: \emph{forward},
\emph{backward} and \emph{central} finite differentiation.

We will use the central finite difference method with a
...
V = a\cdot b \cdot c
\] In general, the volume of a non-orthorhombic
\href{https://en.wikipedia.org/wiki/Crystal_system}{crystal systems} is
calculated as: \[
...
\pandocbounded{\includegraphics[keepaspectratio]{exercise_b/analysis_b_files/figure-pdf/fig-lattice-output-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Experimental Measurement Data is available from:}, opacityback=0, colback=white]
...
type \(b\) at distance \(r\) away from a reference particle type \(a\);
\emph{e.g.} the probability to find \(\mathrm{C}\mathrm{O}_2\) from
\(\mathrm{Zn}^{2+}\) clusters at distance \(r\): \[
P_{ab}(r) = \int_{0}^{r'} 4\pi r'^2 g_{ab}(r') dr'
\] where \(g_{ab}(r)\) is the RDF. The RDF formulates the average local
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-tip-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-tip-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Dirac-Delta Function}, opacityback=0, colback=white]
...
which can also written as the equivalent \textbf{Einstein} expression
...
The Einstein Relation expresses the self-diffustion coefficient as the
slope of the mean-squared-displacement (\(MSD(\tau)\)) with respect to
the lag time \(\tau\)

\[
D_{\mathrm{s}} = \frac{1}{2 d} \lim _{\tau \to \infty} \frac{d}{d \tau} MSD(\tau)
...
The MSD describes the mean squared displacement of particles over a lag
time \(\tau\), averaged over all time origins \(t_0\) \[
MSD(\tau) = \left < \frac{1}{N} \sum_{i=1}^{N} |\vec{r} _{i}(t_0 + \tau) - \vec{r} _{i}(t_0)|^2  \right >_{t_0}
\] where \(N\) is the number of particles and \(\vec{r}_{i}\) is the
center-of-mass position vector of particle \(i\).

Looking at Figure Figure~\ref{fig-msd} (a), we see that the beginning of
the MSD is not linear. Only at higher correlation times the expected
diffusion (see Figure Figure~\ref{fig-msd} (b)) behaviour can be
observed.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-tip-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-tip-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Question:}, opacityback=0, colback=white]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, bottomtitle=1mm, leftrule=.75mm, opacitybacktitle=0.6, rightrule=.15mm, arc=.35mm, colbacktitle=quarto-callout-caution-color!10!white, toprule=.15mm, toptitle=1mm, colframe=quarto-callout-caution-color-frame, titlerule=0mm, left=2mm, bottomrule=.15mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Answer:}, opacityback=0, colback=white]

The self-diffusion coefficient is the slope of the MSD divided by 2
\(\cdot\;d\).

But this is only true if the correlation time is long enough. We can say
that in the linear regime we fulfill this limes due to the constant
slope, which represents the diffusion coefficient.

Therefore, in order to apply the Einstein relation, it is important to
be in the linear regime of the MSD plot.

You get the slope by fitting a \href{../guides/statistics.qmd}{linear
regression} to the linear part of the dataset.
...
\NormalTok{axs[}\DecValTok{0}\NormalTok{].text(}\FloatTok{1.0}\NormalTok{,}\DecValTok{400}\NormalTok{, }\VerbatimStringTok{r"$D\_s=}\SpecialCharTok{\%2.2f}\VerbatimStringTok{\textbackslash{},\textbackslash{}mathrm\{\textbackslash{}AA² \textbackslash{}cdot ps⁻¹\}$"} \OperatorTok{\%}\NormalTok{(d\_einstein),fontsize}\OperatorTok{=}\DecValTok{14}\NormalTok{)}
...
\NormalTok{axs[}\DecValTok{0}\NormalTok{].set\_ylabel(}\VerbatimStringTok{r"$MSD(\textbackslash{}tau)$ in $\textbackslash{}mathrm\{\textbackslash{}AA\^{}2\}$"}\NormalTok{)}
...
\NormalTok{axs[}\DecValTok{1}\NormalTok{].set\_ylabel(}\VerbatimStringTok{r"$MSD(\textbackslash{}tau)$ in $\textbackslash{}mathrm\{\textbackslash{}AA\^{}2\}$"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{exercise_b/analysis_b_files/figure-pdf/fig-msd-output-1.pdf}}
...
velocity-autocorrelation function \(VACF(\tau)\) to get the
self-diffusion coefficent \(D_{\mathrm{s}}\) \[
D_{\mathrm{s}} = \frac{1}{d} \int _{0}^{\infty} VACF(\tau)  d\tau
\] where \(d=1,2,3\) is the dimension.

\subsubsection{Veloctiy-Autocorrelation
Function}\label{veloctiy-autocorrelation-function}

The \href{}{velocity-autocorrelation function} \(VACF(\tau)\) is
averaged over a time interval \(\tau\) \[
VACF(\tau) = \left < \frac{1}{N} \sum_{i=1}^{N} \vec{v} _{i}(t_0) \cdot \vec{v} _{i}(t_0 + \tau) \right >_{t_0}
\] where \(\vec{v}_{i}\) is the velocity of particle \(i\) and \(t_0\)
labels the time origin. \(\tau\) is also called the lag time.
...
\NormalTok{axs.text(}\FloatTok{0.05}\NormalTok{, }\FloatTok{0.95}\NormalTok{, }\VerbatimStringTok{r"$D\_s = }\SpecialCharTok{\%.2f}\VerbatimStringTok{ \textbackslash{}, \textbackslash{}mathrm\{\textbackslash{}AA² \textbackslash{}cdot ps⁻¹\}$"} \OperatorTok{\%}\NormalTok{(d\_green\_kubo), transform}\OperatorTok{=}\NormalTok{axs.transAxes, fontsize}\OperatorTok{=}\DecValTok{14}\NormalTok{,verticalalignment}\OperatorTok{=}\StringTok{\textquotesingle{}top\textquotesingle{}}\NormalTok{)}


\NormalTok{axs.set\_xlabel(}\VerbatimStringTok{r"correlation time $\textbackslash{}tau$ in ps"}\NormalTok{)}
\NormalTok{axs.set\_ylabel(}\VerbatimStringTok{r"$VACF(\textbackslash{}tau)$ in $\textbackslash{}mathrm\{\textbackslash{}AA² \textbackslash{}cdot ps⁻²\}$"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{exercise_b/analysis_b_files/figure-pdf/fig-vacf-output-1.pdf}}

}

\caption{\label{fig-vacf}VACF(τ) plot}
...
\(E_{\mathrm{a}}\) by an exponential decay \[
D_{\mathrm{s}}=D_0 \cdot e^{-\frac{E_{\mathrm{a}}}{RT}}
\] where \(D_0\) is the pre-exponential factor,
\(R=8.3145\,\mathrm{JK^{-1}mol^{-1}}\) is the ideal gas constant and
\(T\) is the temperature.

With this formalism we can apply an
\href{https://en.wikipedia.org/wiki/Arrhenius_equation}{Arrhenius
equation} to \href{../guides/statistics.qmd}{fit a linear function}.
From the slope of the fit the activation energy \(E_{\mathrm{a}}\) can
be calculated. \[
...
\pandocbounded{\includegraphics[keepaspectratio]{exercise_b/analysis_b_files/figure-pdf/fig-ea-output-1.pdf}}
...
All data generated along with a discussion of results fit easily on a
single page. The images and graphs to be plotted are grouped into three
...
\item[$\square$]
  A comparison of the calculated linear thermal expansion coefficient of
  ZIF-8 with reported literature data (include the source and mention
  the method they have used!)
\item[$\square$]
  A graph showing the Arrhenius plot of the Einstein diffusion
  coefficient for the assigned loading at the five different
  temperatures (298.15~K - 398.15~K) including a fit
\item[$\square$]
...
\pandocbounded{\includegraphics[keepaspectratio]{templates/matplotlib_files/figure-pdf/cell-3-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{templates/matplotlib_files/figure-pdf/cell-4-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{templates/matplotlib_files/figure-pdf/cell-7-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{templates/matplotlib_files/figure-pdf/cell-9-output-1.pdf}}
...
\begin{verbatim}
k =  3.000938483850081  +/-  0.0015568452497352744
r0 =  5.0035993438575215  +/-  0.0019241409505812693
R2 =  0.9998566779185891
\end{verbatim}

\pandocbounded{\includegraphics[keepaspectratio]{templates/matplotlib_files/figure-pdf/cell-10-output-2.pdf}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{cd} /path/to/directory & Change to directory \\
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{guides/../img/vim_cheat_sheet.png}
...
{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{guides/../img/ws_plot.png}
...
That's it for the \texttt{python} tutorial!
...
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{guides/../img/anatomy.png}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-4-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-5-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-7-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-8-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-9-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-10-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-11-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-13-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-14-output-1.pdf}}
...
\CommentTok{\#plt.savefig(\textquotesingle{}../data/test.png\textquotesingle{}, dpi=300, bbox\_inches=\textquotesingle{}tight\textquotesingle{}) }
\NormalTok{plt.show()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-15-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-16-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-17-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-19-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-20-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-21-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-23-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-24-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{guides/VisualizationPythonPackages_files/figure-pdf/cell-25-output-1.pdf}}
