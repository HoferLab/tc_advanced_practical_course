...
The folders encoded in the varible `ZIF_DIR` contains all necessary files for the simulations. The files are organized in the following way:
...
**2.** Add the CO2 loading to the topology and restart files (e.g. 32 carbon dioxide molecules and 298 K)

```bash
add_molecules zif8_preeq_298.rst co2.rst   \
...
## Extract Carbon Dioxide Molecule {.unnumbered}

Extract the carbon dioxide molecule from the trajectory and velocity files using the `extract_co2` tool:
...
- Render the image: `File > Render > Tachyon (internal, in-memory rendering)`
