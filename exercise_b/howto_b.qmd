...
- [Thermal Expansion](https://docs.google.com/spreadsheets/d/1uy_fF13Oas9IhqCImCSvfEXV6vRb6oteAZK-Uj8DyMA/edit?usp=sharing)
- [Gas Diffusion](https://docs.google.com/spreadsheets/d/1zJnp6O_ToFMoJD91_3kiozHoKz9uGf1wEIlJoa3D_U8/edit?usp=sharing)
...
**2.** Add the CO~2~ loading to the restart file (e.g. 32 carbon dioxide molecules with a minimal intermolecular distance of 2.0 Å at 298 K)

```bash
add_molecules zif8_preeq_298.rst co2.rst --rst-mol-desc-file moldescriptor.dat --cut 2.0 -n 32 > 32xco2-zif8-00.rst
```
...
**2.** Calculate the RDF for `Zn` and `C` atoms (gas) using the `rdf_zif8` tool:

```bash
rdf_zif8 <trajectory_file-03>.xyz <trajectory_file-04>.xyz --center Zn --ligand1 C
```

:::{.callout-note}
The RDF calculation will generate `output` files named `rdf-C-C.out` and `rdf-Zn-C.out`, accordingly. Plot the 1st column (distance) against the 2nd column (RDF) using your preferred plotting tool.
:::
