...
## Running Average
The running average $\left < A \right >_{\tau}$ at time $\tau$ is calculated by averaging the previous $\omega$ values, sampled at intervals of $g$:

$$
\left < A \right >_{\tau} = \frac{1}{\omega}\sum_{i=0}^{\omega - 1} a(\tau - i \cdot g)
$$
*e.g.* $\tau=10$, $\omega=3$, $g=2$
$$
\left < A \right >_{10} = \frac{1}{3} (a(t_{10})+a(t_{8})+a(t_{6}))
$$

It smooths the signal by reducing short-term fluctuations while retaining the overall trend of the data.

## Cumulative Average

At each step, the cumulative average takes the previous average $\left < A \right >_{n-1}$ and adjusts it to account for the new data point $x_n$:
$$
...
For further detail: [statistics](../guides/statistics.qmd)
...
V = a\cdot b \cdot c
$$
In general, the volume of a non-orthorhombic [crystal systems](https://en.wikipedia.org/wiki/Crystal_system) is calculated as:
$$
...
The Einstein Relation expresses the self-diffustion coefficient as the slope of the mean-squared-displacement ($MSD(\tau)$) with respect to the lag time $\tau$
...
The MSD describes the mean squared displacement of particles over a lag time $\tau$, averaged over all time origins $t_0$
$$
MSD(\tau) = \left < \frac{1}{N} \sum_{i=1}^{N} |\vec{r} _{i}(t_0 + \tau) - \vec{r} _{i}(t_0)|^2  \right >_{t_0}
$$
where $N$ is the number of particles and $\vec{r}_{i}$ is the center-of-mass position vector of particle $i$.
...
Looking at Figure @fig-msd (a), we see that the beginning of the MSD is not linear. Only at higher correlation times the expected diffusion (see Figure @fig-msd (b)) behaviour can be observed.
...
We can say that in the linear regime we fulfill this limes due to the constant slope, which represents the diffusion coefficient.
...
axs[0].text(1.0,400, r"$D_s=%2.2f\,\mathrm{\AA² \cdot ps⁻¹}$" %(d_einstein),fontsize=14)
...
axs[0].set_ylabel(r"$MSD(\tau)$ in $\mathrm{\AA^2}$")

axs[0].set_ylim(0,500)


axs[1].set_xlabel(r"correlation time $\tau$ in ps")
axs[1].set_ylabel(r"$MSD(\tau)$ in $\mathrm{\AA^2}$")
axs[1].set_xlim(0,5)
...
The Green-Kubo formalism needs a numerical integration of the velocity-autocorrelation function $VACF(\tau)$ to get the self-diffusion coefficent $D_{\mathrm{s}}$
$$
...
$$
where $\vec{v}_{i}$ is the velocity of particle $i$ and $t_0$ labels the time origin. $\tau$ is also called the lag time.
...
axs.text(0.05, 0.95, r"$D_s = %.2f \, \mathrm{\AA² \cdot ps⁻¹}$" %(d_green_kubo), transform=axs.transAxes, fontsize=14,verticalalignment='top')


axs.set_xlabel(r"correlation time $\tau$ in ps")
axs.set_ylabel(r"$VACF(\tau)$ in $\mathrm{\AA² \cdot ps⁻²}$")
...
The diffusion coefficient is directly depending on the activation energy $E_{\mathrm{a}}$ by an exponential decay
$$
D_{\mathrm{s}}=D_0 \cdot e^{-\frac{E_{\mathrm{a}}}{RT}}
$$
where $D_0$ is the pre-exponential factor, $R=8.3145\,\mathrm{JK^{-1}mol^{-1}}$ is the ideal gas constant and $T$ is the temperature.

With this formalism we can apply an [Arrhenius equation](https://en.wikipedia.org/wiki/Arrhenius_equation) to [fit a linear function](../guides/statistics.qmd). From the slope of the fit the activation energy $E_{\mathrm{a}}$ can be calculated.
$$
