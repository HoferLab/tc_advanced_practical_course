---
title: "Analysis B"
subtitle: "Analysis Techniques for Computational Material Science"
format: 
...
It is a good sign if 
 ...
 - [ ] no unintended bond breaking or formation occurs

 - [ ] system remains intact -> no collapsing or blowing up

 - [ ] no translation of the whole box or super fast spinning of a molecule
...
- [ ] Try to understand any error messages that may have been raised


- [ ] Check the `*.log` and `*.in` file to see if the simulation was set up properly


- [ ] Check the starting structure


- [ ] Check the moldescriptor file and shake files
...
## Arithmetic Average, Standard Deviation and Standard Error {visibility="hidden"}
...
## Running Average {visibility="hidden"}
...
## Cummulative Average {visibility="hidden"}
...
## Linear Regression Fit {visibility="hidden"}
...
## Plot of Temperature over Simulation Time {auto-animate="true"}
...
There are [three](https://en.wikipedia.org/wiki/Finite_difference) fundamental types of this method, the *forward*, the  *backward* and the *central* finite difference. 


## Linear Thermal Expansion Coefficient


In our case we use the central finite difference method with a **[five-point stencil](https://en.wikipedia.org/wiki/Five-point_stencil)**.
In general it is formulated in 1D as:

$$
\begin{align}
f'(x) = \frac{-f(x+2h)+8f(x+h)-8f(x-h)+f(x-2h)}{12 h} + \\
 +\frac{h^4}{30}f^{(5)(c)}, c \in [x-2h,x+2h]
\end{align}
$$
...
If more than five points are used, a [higher](https://en.wikipedia.org/wiki/Finite_difference_coefficient) order stencil can be employed.
...
The volume of an orthorhombic system can be calculated as:
$$
V = a\cdot b \cdot c
$$

In the general case of a non-orthorhombic [crystal sytems](https://en.wikipedia.org/wiki/Crystal_system) the volume is calculated as:
$$
...
plt.xlabel(r"$T$ / K")
plt.ylabel(r"$\left < a \right > $ / $\mathrm{\AA}$")
plt.xlim(223,373)
...
![](../img/rdf_sketch.png){width=25%}

## Radial Distribution Functions (RDFs) 

It describes the probability $P_{ab}(r)$ to find a target particle of type $b$ at distance $r$ from a reference particle type $a$; *e.g.* the probability to find $\mathrm{H}_2$ molecules at distance $r$ from $\mathrm{Zn}^{2+}$:
$$
...
In order to describe the dynamics of guest molecules in a host system, it is necessary to calculate transport properties. 
A transport property would be for example the *diffusion*, *viscosity*, *electrical or thermal conductivity*.

In general transport properties can be described as a property coefficient $\gamma$ which depends on a microscopic variable $A$ (*e.g.* positions $\Delta \vec{r}$ or the velocities $\vec{v}$) that is written as an infinite time integration of a non-normalized time correlation function $\left < \dot{A}(t)\dot{A}(t_0) \right >$ :
$$
...
In case of the self-diffusion coefficient $D_{\mathrm{s}}$, the variable $A$ is the molecular position $\vec{r}_{\mathrm{cm}}$, which in general is the center of mass position of the particles:
$$
...
The mean-squared displacement describes the displacement of the particle from a time origin $t_0$ averaged over the time interval $\tau$
$$
MSD(\tau) = \left < \frac{1}{N} \sum_{i=1}^{N} |\vec{r} _{i}(t) - \vec{r} _{i}(t_0)|^2  \right >_{\tau}
$$
where $N$ is the number of particles of that type and $\vec{r}_{i}$ is the center-of-mass position vector of particle $i$.
...
Looking at Figure @fig-msd (a), we see that the beginning of the MSD is not linear. Only at higher correleation times the expected diffusion (see Figure @fig-msd (b)) behavior is exhibited.
...
But this is only true if correlation time is long enough.
We can say that in the linear regime we fulfill this limes due to unchanging slope which represents the difffusion coefficient.

Therefore, it is really important to be in the regime of linear behavior, in order to apply the Einstein relation.

You get the slope by fitting a [linear regression](../guides/statistics.qmd) at the last linear section.
:::
...
fig, axs = plt.subplots(2,1,figsize=(2,5),gridspec_kw={'hspace': 0.4})
...
axs[0].set_xlabel(r"correlation time $\tau$ / ps")
axs[0].set_ylabel(r"$MSD(t)$ / $\mathrm{\AA^2}$")

axs[0].set_ylim(0,500)


axs[1].set_xlabel(r"correlation time $\tau$ / ps")
axs[1].set_ylabel(r"$MSD(t)$ / $\mathrm{\AA^2}$")
axs[1].set_xlim(0,5)
...
The Green-Kubo formalism requires numerical integration of the velocity-autocorrelation function $VACF(t)$ to get the self-diffusion coefficent $D_{\mathrm{s}}$
$$
...
$$
where $\vec{v}_{i}$ is the velocity the particle $i$, and $t_0$ labels the time origin.
...
$$
where $D_0$ is a constant factorn, $R=8.3145\,\mathrm{JK^{-1}mol^{-1}}$ is the ideal gas constant and $T$ is the temperature.
